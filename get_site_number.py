...
class getSiteNumber(object):
	...
	def get_number(self):
		...
		thread = unirest.get(self.url, callback=self.crawl_page)

	def crawl_page(self,response):
		# print(response.body)
		soup = BeautifulSoup(response.raw_body, "lxml")
		if not soup.body:
			return None
		...
		if len(self.possibleNumbers) < 1:
			for link in self.page_links:
				if not link in self.gone_through:
					self.url = link
					self.get_number()
